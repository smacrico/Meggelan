...
class RecalculationService:
    ...
    def recompute_all(self, source_name: str) -> dict[str, int]:
        ...
        if not measurements:
            return {"measurements": 0, "baselines": 0, "trends": 0, "alerts": 0, "anomalies": 0}

        baseline_row = self._insert_baseline(source_name, measurements)
        trend_count = self._insert_trends(source_name, measurements)
        alert_count = self._insert_alerts(source_name, measurements, baseline_row)
        anomaly_count = self._insert_anomalies(source_name, measurements, baseline_row)
    ...
    def _get_measurements(self, source_name: str) -> list[HRVMeasurement]:
        stmt = (
            ...
            .order_by(desc(HRVMeasurement.measurement_date), desc(HRVMeasurement.id))
        )
        return list(self.session.execute(stmt).scalars().all())

    def _insert_baseline(self, source_name: str, measurements: list[HRVMeasurement]) -> HRVBaseline:
        values = {
            metric: [float(getattr(m, metric) or 0.0) for m in measurements]
            for metric in METRICS
        }

        latest_date = measurements[0].measurement_date
    ...
    def _insert_trends(self, source_name: str, measurements: list[HRVMeasurement]) -> int:
    ...
    def _insert_alerts(
        self,
        source_name: str,
        measurements: list[HRVMeasurement],
