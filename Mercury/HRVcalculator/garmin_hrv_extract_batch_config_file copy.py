...
def make_daily_trends(df: pd.DataFrame) -> pd.DataFrame:
    ...
    daily = (
        out.groupby(out["date"].dt.strftime("%Y-%m-%d"), as_index=False)
        .agg(
            sessions=("file", "count"),
            avg_rmssd=("rmssd", "mean"),
            avg_sdnn=("sdnn", "mean"),
        ...
        .rename(columns={"date": "summary_date"})
        .sort_values("date" if "date" in out.columns else out.columns[0])
    )

    # ensure exact column name
    if daily.columns[0] != "summary_date":
        daily = daily.rename(columns={daily.columns[0]: "summary_date"})
