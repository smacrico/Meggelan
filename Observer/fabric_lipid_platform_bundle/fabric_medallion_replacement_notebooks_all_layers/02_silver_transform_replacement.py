...
bronze_table_root = make_onelake_table_root(workspace_name_or_id, bronze_lakehouse_name)
bronze_raw_table_path = table_path(bronze_table_root, bronze_raw_table_name)
...
print(f"  bronze_raw_table_path      : {bronze_raw_table_path}")
...
def parse_date_expr(column_expr):
    ...
    - Common dd/MM/yyyy and MM/dd/yyyy strings
    - Excel serial dates
    """
    raw = F.trim(column_expr.cast("string"))
    ...
    return F.coalesce(
        ...
        F.to_date(raw, "yyyy-MM-dd'T'HH:mm:ss"),
...
# COMMAND ----------
# READ BRONZE TABLE BY ABSOLUTE ONELAKE PATH

bronze_raw = spark.read.format("delta").load(bronze_raw_table_path)
