...
void TrySetCalcGroupPrecedence(CalculationGroupTable cg, int precedence)
{
    try
    {
        // Some TE builds expose an Annotation-backed or metadata-backed path only.
        var prop = cg.GetType().GetProperty("Precedence");
        if(prop != null && prop.CanWrite)
        {
            prop.SetValue(cg, precedence, null);
            return;
        }
    }
    catch
    ...
    try
    {
        // harmless fallback marker for documentation if property is unavailable
        cg.SetAnnotation("IntendedPrecedence", precedence.ToString());
    }
    catch
    {
    }
...
TrySetCalcGroupPrecedence(cg, 20);
...
AddOrReplaceMeasure(lipidTable, "Lipid TC/HDL Color Hex", @"
SWITCH(
    [Lipid TC/HDL Score],
    1, ""#2E7D32"",
    2, ""#F9A825"",
...
AddOrReplaceMeasure(lipidTable, "Lipid LDL/HDL Color Hex", @"
SWITCH(
    [Lipid LDL/HDL Score],
    1, ""#2E7D32"",
    2, ""#F9A825"",
...
AddOrReplaceMeasure(lipidTable, "Lipid Non-HDL Color Hex", @"
SWITCH(
    [Lipid Non-HDL Score],
    1, ""#2E7D32"",
    2, ""#F9A825"",
...
AddOrReplaceMeasure(endoTable, "Endo HbA1c Color Hex", @"
SWITCH(
    [Endo HbA1c Score],
    1, ""#2E7D32"",
    2, ""#F9A825"",
...
AddOrReplaceMeasure(endoTable, "Endo Glucose Color Hex", @"
SWITCH(
    [Endo Glucose Score],
    1, ""#2E7D32"",
    2, ""#F9A825"",
...
AddOrReplaceMeasure(lipidTable, "Lipid AIP Trend Color Hex", @"
SWITCH(
...
AddOrReplaceMeasure(lipidTable, "Lipid TG/HDL Trend Color Hex", @"
SWITCH(
...
AddOrReplaceMeasure(endoTable, "Endo HOMA-IR Trend Color Hex", @"
SWITCH(
...
AddOrReplaceMeasure(endoTable, "Endo QUICKI Trend Color Hex", @"
SWITCH(
...
AddOrReplaceMeasure(lipidTable, "Combined KPI Status Color Hex", @"
...
Info("TE2-safe patched calculation group and KPI color measures created/updated successfully.");