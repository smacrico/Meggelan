...
def safe_latest_value(row: pd.Series, column: str, default: float = 0.0) -> float:
    try:
...
def enhanced_metric_summary(frame: pd.DataFrame) -> pd.DataFrame:
    metric_labels = {
        "acute_load_7d": "Acute Load 7d",
        "chronic_load_28d": "Chronic Load 28d",
        "acwr_7_28": "ACWR 7/28",
        "weekly_monotony": "Weekly Monotony",
        "weekly_strain": "Weekly Strain",
        "efficiency_factor": "Efficiency Factor",
        "performance_index": "Performance Index",
        "aerobic_decoupling_pct": "Aerobic Decoupling %",
        "physio_efficiency": "Physio Efficiency",
        "fatigue_index": "Fatigue Index",
        "speed_consistency": "Speed Consistency",
        "speed_vo2max_index": "Speed x VO2max Index",
    }
    rows = []
    for col, label in metric_labels.items():
        ...
        rows.append({
            ...
            "min": series.min(),
            "max": series.max(),
            "std": 0.0 if pd.isna(series.std()) else series.std(),
        })
    return pd.DataFrame(rows)


def add_metric_help():
    with st.expander("Enhanced metric guide"):
        st.markdown(
            """
            **ACWR 7/28** compares the last 7 days of load with the 28-day chronic-load baseline. Values around 0.8–1.3 are generally treated as a steadier zone in this dashboard.

            **Weekly monotony** increases when training load is very similar every day. Higher values can indicate less variation.

            **Weekly strain** combines weekly load and monotony, highlighting hard weeks that also lack variation.

            **Efficiency factor** is speed divided by heart rate. Higher values usually mean more speed for the same cardiovascular cost.

            **Performance index** combines speed, running economy, and heart rate.

            **Aerobic decoupling %** uses first-half/second-half HR-speed data when present; otherwise it falls back to your cardiac-drift proxy.
            """
        )
...
with tabs[5]:
    ...
    summary_df = enhanced_metric_summary(filtered)
    st.subheader("Enhanced Metrics Summary")
    st.dataframe(summary_df, use_container_width=True)

    latest_cols = existing_columns(filtered, [
        "date", "acute_load_7d", "chronic_load_28d", "acwr_7_28",
        "weekly_monotony", "weekly_strain", "efficiency_factor",
        "performance_index", "aerobic_decoupling_pct", "physio_efficiency",
        "fatigue_index", "recovery_score", "readiness_score", "risk_level",
    ])
    st.subheader("Latest Enhanced Session Values")
    st.dataframe(filtered[latest_cols].tail(10).sort_values("date", ascending=False), use_container_width=True)

    c1, c2 = st.columns(2)
    with c1:
        monthly_flat_enhanced = flatten_monthly(filtered_monthly)
        monthly_cols = existing_columns(monthly_flat_enhanced, [
            "year_month", "acwr_7_28_mean", "weekly_monotony_mean", "weekly_strain_mean",
            "efficiency_factor_mean", "performance_index_mean", "aerobic_decoupling_pct_mean",
        ])
        if monthly_cols:
            st.subheader("Monthly Enhanced Averages")
            st.dataframe(monthly_flat_enhanced[monthly_cols], use_container_width=True)

    with c2:
        risk_cols = existing_columns(filtered, [
            "date", "risk_level", "fatigue_flag", "overtraining_flag",
            "acwr_7_28", "weekly_monotony", "weekly_strain", "fatigue_index",
            "recovery_score", "readiness_score",
        ])
        st.subheader("Enhanced Risk Detail")
        st.dataframe(filtered[risk_cols].sort_values("date", ascending=False), use_container_width=True)

    c3, c4 = st.columns(2)
    with c3:
        if {"efficiency_factor", "heart_rate", "avg_speed"}.issubset(filtered.columns):
            fig_ef_scatter = px.scatter(
                filtered,
                x="heart_rate",
                y="efficiency_factor",
                size="avg_speed",
                color="risk_level",
                hover_data=["date", "avg_speed", "TRIMP"],
                title="Efficiency Factor vs Heart Rate",
            )
            st.plotly_chart(fig_ef_scatter, use_container_width=True)
            figures["Efficiency Factor vs Heart Rate"] = fig_ef_scatter

    with c4:
        if {"performance_index", "TRIMP"}.issubset(filtered.columns):
            fig_perf_load = px.scatter(
                filtered,
                x="TRIMP",
                y="performance_index",
                color="risk_level",
                hover_data=["date", "avg_speed", "heart_rate"],
                title="Performance Index vs Training Load",
            )
            st.plotly_chart(fig_perf_load, use_container_width=True)
            figures["Performance Index vs Training Load"] = fig_perf_load
...
with tabs[7]:
    ...
    if st.button("Save enhanced training log, monthly summaries, and metrics breakdown to SQLite"):
        ...
        st.success("Saved enhanced outputs to the configured enhanced SQLite tables.")
    ...
    with export_col1:
        if st.button("Export Full HTML Dashboard"):
            ...
            tables = {
                ...
                "Enhanced Metrics Summary": enhanced_metric_summary(filtered),
